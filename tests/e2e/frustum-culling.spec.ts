 ...
 * 
 * Tests actual rendering with real camera frustum calculations.
...
for (const rendererType of E2E_RENDERER_TYPES) {
  test.describe(`[${rendererType.toUpperCase()}] Frustum Culling E2E`, () => {
   ...
   */
  test.describe(`[${rendererType.toUpperCase()}] Deterministic Frustum Culling`, () => {
    test.beforeEach(async ({ page }) => {
      await setupScene(page, rendererType);
    });
    ...
    test('should correctly handle instances at frustum near/far boundaries', async ({ page }) => {
      const result = await page.evaluate(() => {
        ...
        // Use positions well inside/outside bounds to avoid floating-point edge cases
        mesh.addInstances(6, (obj, index) => {
          switch (index) {
            case 0: obj.position.set(0, 0, -2); break;      // Well before near plane - CULLED
            case 1: obj.position.set(0, 0, -15); break;     // Inside frustum - VISIBLE
            case 2: obj.position.set(0, 0, -50); break;     // Middle of frustum - VISIBLE
            case 3: obj.position.set(0, 0, -90); break;     // Inside frustum - VISIBLE
            case 4: obj.position.set(0, 0, -200); break;    // Well beyond far plane - CULLED
